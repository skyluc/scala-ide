...
import org.scalaide.util.internal.swt.TableCombo
...
class NewFileWizard2 extends BasicNewResourceWizard {
  
  var page: NewFilePage2 = _
  ...
  override def init(workbench: org.eclipse.ui.IWorkbench, selection: org.eclipse.jface.viewers.IStructuredSelection) {
    super.init(workbench, selection)
  }
    
  
  override def addPages() {
    page = new NewFilePage2
    addPage(page)
  }

}

class NewFilePage2 extends WizardPage("Create File") with HasLogger {
  
  private val fileCreatorId = WizardHandler.DefaultTypeCreator
  ...
  def createControl(parent: Composite): Unit = {
    val Red = new Color(getShell().getDisplay(), 255, 0, 0)
    
    disposables = Seq[{def dispose(): Unit}](Red)
    ...
    btProject.addSelectionListener { e: SelectionEvent =>
      Dialogs.chooseProjectWithDialog(getShell) foreach { p =>
        selectedProject = p
  ...
  private def openEditor(path: IPath)(f: ITextViewer => Unit): Unit = {
    ...
    try {
      ...
      case e: PartInitException =>
        logger.error(s"Failed to initialize editor for file '$file'", e)
        None
    }
  }
