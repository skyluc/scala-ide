...
private[logging] object EclipseLogger extends Logger {
  private val pluginLogger: ILog = ScalaPlugin().getLog()
  ...
  private def logInUiThread(severity: Int, message: Any, exception: Throwable): Unit = {
    val status = new Status(severity, ScalaPlugin().getBundle.getSymbolicName, message.toString, exception)
    if (ScalaPlugin().headlessMode) pluginLogger.log(status)
