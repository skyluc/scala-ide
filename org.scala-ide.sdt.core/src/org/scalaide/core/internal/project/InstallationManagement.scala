...
import org.scalaide.core.api.SdtConstants
...
trait InstallationManagement { this: ScalaProject =>
  ...
  def desiredSourceLevel(): String = {
    implicit val sourceLevelDefault = ScalaPlugin().shortScalaVersion
  ...
  def effectiveScalaInstallation(): LabeledScalaInstallation = {
    implicit val desiredInstallationDefault: LabeledScalaInstallation = ScalaInstallation.resolve(ScalaInstallationChoice(ScalaPlugin().scalaVersion)).get
  ...
  def setDesiredSourceLevel(scalaVersion: ScalaVersion = ScalaVersion(desiredSourceLevel()),
    ...
    if (isUsingCompatibilityMode()) {
      if (CompilerUtils.isBinarySame(ScalaPlugin().scalaVersion, scalaVersion)) {
        unSetXSourceAndMaybeUntoggleProjectSettings(slReason)
      }
    } else {
      if (CompilerUtils.isBinaryPrevious(ScalaPlugin().scalaVersion, scalaVersion)) {
  ...
  private def unSetXSourceAndMaybeUntoggleProjectSettings(reason: String) = {
    if (usesProjectSettings) { // if no project-specific settings, Xsource is ineffective anyway
      ...
      val projectSettingsSameAsWorkSpace = shownSettings(ScalaPresentationCompiler.defaultScalaSettings(), _ => true) forall {
        case (setting, value) => ScalaPlugin().getPreferenceStore().getString(SettingConverterUtil.convertNameToProperty(setting.name)) == value
      }
      val scalaInstallationIsSameAsDefault = {
        val desiredInstallChoice = desiredinstallationChoice()
        desiredInstallChoice.marker match {
          case Left(scalaVersion) => CompilerUtils.isBinarySame(ScalaPlugin().scalaVersion, scalaVersion)
          case Right(_) => false
        }
      }
  ...
  */
  private def getCompatibilityMode(): Boolean = {
    ...
    if (specdVersion exists (ScalaVersion(_) > ScalaPlugin().scalaVersion))
      eclipseLog.error(s"Incompatible Xsource setting found in Compiler options: $specdVersion")
    if (l < 1 || (specdVersion exists (x => CompilerUtils.isBinarySame(ScalaPlugin().scalaVersion, ScalaVersion(x)))))
      false
    else
      specdVersion exists (x => CompilerUtils.isBinaryPrevious(ScalaPlugin().scalaVersion, ScalaVersion(x)))
  }
