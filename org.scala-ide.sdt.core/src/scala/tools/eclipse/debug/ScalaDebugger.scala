...
object ScalaDebugger extends IJavaBreakpointListener with IJDIEventListener {
  ...
  def stepOver(stackFrame: JDIStackFrame) {
    ...
    val request= stackFrame.getJavaDebugTarget.createClassPrepareRequest(stackFrame.getDeclaringTypeName + '*')
    stackFrame.addJDIEventListener(this, request)
    
    val closuresInRange= (method.declaringType.nestedTypes).asScala.flatten(_.methods.asScala).filter(m => {
      val minLine= (methodToLines(m) :+ Int.MaxValue).min
      minLine >= currentLine && minLine < nextMethodFirstLine && m.name.startsWith("apply$")
    })
    
    
    val attributes= Map(IBreakpoint.PERSISTED -> false)
