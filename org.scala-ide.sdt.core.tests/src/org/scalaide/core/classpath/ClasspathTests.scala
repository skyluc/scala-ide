...
class ClasspathTests {
  ...
  private def prefStore = ScalaPlugin().getPreferenceStore()
  ...
  private val testShortPreviousScalaVersion: String = CompilerUtils.previousShortString(ScalaPlugin().scalaVersion)

  // for these tests' purposes of comparing minors, it's enough to get "none" if the plugin version is unparseable
  private val testShortScalaVersion: String = ScalaPlugin().shortScalaVersion
