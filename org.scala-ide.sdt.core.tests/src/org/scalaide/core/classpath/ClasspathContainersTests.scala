...
class ClasspathContainersTests {
  ...
  val currentScalaVer = ScalaPlugin().scalaVersion match {
  ...
  val previousScalaVer = ScalaPlugin().scalaVersion match {
      case CompilerUtils.ShortScalaVersion(major, minor) => {
  ...
  def deleteProjects() {
    EclipseUtils.workspaceRunnableIn(EclipseUtils.workspaceRoot.getWorkspace) { _ =>
      projects foreach { project =>
        ...
        (new File(ScalaPlugin().getStateLocation().toFile(), project.underlying.getName + new Path(libraryId).toPortableString() + ".container")).delete()
      }
    }
    projects = List()
  }
  ...
  def source_level_reversal_reverses_container_to_older() {
    ...
    project.setDesiredSourceLevel(ScalaPlugin().scalaVersion, "explicit initialization of source_level_reversal_to_older")
    ...
    project.setDesiredSourceLevel(ScalaPlugin().scalaVersion, reversalReason)
  ...
  def source_level_reversal_reverses_container_to_newer() {
    if (ScalaPlugin().scalaVersion >= ScalaVersion("2.11.0")) {val project = createProject()
