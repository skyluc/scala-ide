...
class CompilerSettingsTest {
  ...
  def clean_deprecation_and_additional() {
    ...
    val projectStore = new ScopedPreferenceStore(projectScope, ScalaConstants.PluginId)
  ...
  def no_import_from_propertystore_to_instance_scope() {
    setProjectSettings("deprecation", "true")
    val instanceStore = new ScopedPreferenceStore(InstanceScope.INSTANCE, ScalaConstants.PluginId)
  ...
  private def enableProjectSettings(value: Boolean = true) {
    val projectStore = new PropertyStore(projectScope, ScalaConstants.PluginId)
